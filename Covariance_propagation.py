...
def Ensemble_activation(mu_z, sigma_z, image_size, num_filters): 
    '''
    mu_z shape shape=[1, image_size,image_size,num_filters[0]]
    sigma_z=shape=[num_filter,image_size*image_size, image_size*image_size]
    '''
    ...
    x_hat = tf.transpose(tf.reshape(tf.squeeze(mu_z), [-1, num_filters]))#shape=[num_filter[0], image_size*image_size]
    dist = tfd.MultivariateNormalDiag(
    loc=x_hat ,scale_diag=L_gg)
    ensemble= dist.sample(sample_shape=(n_g), dtype=tf.float32, seed=1) #shape=[n_g,num_filter[0], image_size*image_size]
    ensemble_trans= tf.nn.elu(ensemble)

    mu_g1 = tf.reduce_mean(ensemble_trans, 0) #shape=[num_filter[0], image_size*image_size]
    ...
    P_gg = tfp.stats.covariance(ensemble_trans, sample_axis=0, event_axis=-1)
    #shape=[num_filter[0], image_size*image_size, image_size*image_size]
    return mu_g, P_gg
